...
class KeyCloak {
	...
	public function __construct ($config_data) {
		...
		if (!$this->is_public) {
			...
			$this->secret = array_key_exists('client-secret', $config_data) ? $config_data['client-secret'] : "dummy";
		}
		 ...
		 * @type {String}
		 */
		$this->realm_url = $auth_server_url . 'auth/realms/' . $this->realm_id;
		...
		$this->redirect_uri = $config_data['redirect_uri'] ?? '';
	...
	**/
	 ...
	 */
	public function send_request ($method = 'GET', $path = '/', $headers = array(), $data = '') {
